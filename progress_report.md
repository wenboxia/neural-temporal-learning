...
**三个方向**（技术规格与执行顺序见 [todo.md](todo.md) P1）：
...
**目标（2026-09-05 更新）**：完成 **KTH 硕士毕业论文答辩，pass 即可**，做得好一点更好。
早期"发一篇期刊文章"的目标已放弃 → 不做投稿版、不需要论证 novelty 强度、不需预演 reviewer 攻击。

**交付物**：一份毕业论文（中文正文 + 英文摘要），完整含 Phase 2/2.5 appendix。

**核心定位**：**待定** —— 取决于 Phase 5.5 能否拿到正面结果。
负面结果保留但降级为方法演进的中间步骤 / 附录，不作核心卖点。
...
## Phase 5.5 — 证据校准（2026-09-06，重启后第一批）

停滞三个月后重启。在启动任何新实验前，先对既有证据做了一轮校准，三处结论被更正
（**原始实验数据全部有效**，被更正的是分析与解释）：

### 更正 1：Insects 漂移坐标用错了

仓库此前使用的 5 个坐标（12,672 / 14,256 / 17,952 / 46,728 / 52,008）来自 2026-04-29 的
50-chunk **P(y) 构成变化**诊断。`src/data/real_world.py` 的原注释写的是"候选位置"，
但后续文档把它们升格成了 "documented drift"。

官方坐标见 Souza et al. 2020 Table 2，"Abrupt (bal.)"，52,848 instances：
**14,352 / 19,500 / 33,240 / 38,682 / 39,510**（由捕虫器温度变化引起，30°C → 20°C → ~35°C → …，
即 P(X\|y) 漂移）。
...
| 坐标组 | 类条件特征偏移 \|Δμ\|/σ |
|---|---|
| 经验 P(y) 点（仓库旧用） | 0.05 – 0.09（全流中位数 ≈ 0.07）|
| 官方温度点 | 0.25 – 0.49 |

且 12,672 与 14,256 都落在同一段连续 class 5（[12,598, 14,352) 共 1,754 条）**内部**，
不是该单类区间的真实边界。

**影响**：B1+ 四段按官方坐标只覆盖 **2/5** 个真实漂移（early 含 14,352，mid 含 19,500）；
late_pre / late_post 不含任何官方漂移。Phase 5 的 "0/20 触发" 有相当部分是在无漂移段上测的。

### 更正 2：γ 诊断的关键数字复现不出

原文称 indicator 位移 \|Δ\| ≤ 0.019、"比合成小 10×"。用同一批 npz 重算：

| 位置 | 5 seeds 的 indicator \|Δ\| |
|---|---|
| 经验点（如 mid local 1,952）| 0.000 – 0.040 |
| **官方点 19,500（mid local 3,500）** | **0.095 – 0.105** |

"10× 稀释" 作废。真实信号约为合成 regime_switching（0.20）的一半，不是十分之一。

### 更正 3：detector 沉默的主因是实现层阈值

`src/drift/error_detector.py` 的自写 ADWIN 用**值域** Hoeffding 界，不用经验方差：

```
ε_cut = value_range · sqrt(0.5 · (1/n0 + 1/n1) · ln(2/δ'))
```

在实际默认配置（max_window = buffer_size×4 = 400、min_subwindow = 30、δ = 0.002、
value_range = 1.0）下，即使最有利的 200/200 切分也要求 \|Δmean\| ≥ **0.209**。
真实 Insects 的 indicator 流错误率只有 2–4%，官方点处位移 ≈ 0.10 → **结构性不可能触发**。

检测器只消费 1D 的 indicator 流，而该流已存在每个 phase4a npz 里，所以换实现是
**零 CPU 成本**的离线重放（`scripts/diag_detector_replay.py`）。对全部 35 条已存流：

| impl | δ | 数据集 | 有报警的 run | 官方漂移 recall | 中位延迟 |
|---|---|---|---|---|---|
| own | 0.002 | insects | 0/20 | 0.00 | — |
| **river** | 0.002 | insects | **16/20** | **1.00** | 266 |
| own | 0.002 | electricity | 1/15 | — | — |
| river | 0.002 | electricity | 1/15 | — | — |

两个落在 B1+ 段内的官方漂移都被 5/5 seed 检出：mid @local 3,500（延迟 145–172 步）、
early @local 4,352（延迟 361–447 步）。Electricity 是渐进漂移，river 下仍近乎沉默，符合预期。
...
检测器现在能触发了 ⇒ 首次具备"验证三层结构是否有用"的条件。但重放同时显示
**检测延迟 145–447 步**，而 TabPFN 靠滑窗自恢复也只需约 100–200 步 ——
所以"报警后再校正"的抢救窗口可能很小。因此下一步不是直接上规模，而是先做
**判别性对照**：oracle 即时触发 vs 实际报警 × {adapter 路由 / context 重置 / 只清 buffer / 无动作}，
在触发后同一批样本上比较累计错误。详见 `todo.md` P1。
