...
> ⚠️ **2026-09-06 Phase 5.5 更正 —— 本文件下列内容已过时，以 [`todo.md`](todo.md) 为准**
>
> 1. **Insects 漂移坐标写错了**。本文件 §3 数据集表里的 12,672 / 14,256 / 17,952 / 46,728 / 52,008
>    是 50-chunk **P(y) 构成变化点**（经验推断），不是官方漂移点。
>    Souza 2020 Table 2 "Abrupt (bal.)" 的官方坐标是 **14,352 / 19,500 / 33,240 / 38,682 / 39,510**。
>    B1+ 四段按官方坐标只覆盖 **2/5** 个真实漂移。
> 2. **§4 履历表里 "detector 仍 0/20" 的机制解释不成立**。沉默的主因是自写 ADWIN 的值域 Hoeffding 界
>    （默认配置要求 \|Δmean\| ≥ 0.209，真实信号只有 0.10），不是 "TabPFN 自适应吃掉信号"。
>    换 river 标准 ADWIN 离线重放同一批已存信号：Insects **16/20 运行报警、官方漂移 recall 1.00**。
> 3. **γ 诊断的 "\|Δ\| ≤ 0.019、10× 稀释" 复现不出**（官方点处实测 0.095–0.105），已作废。
> 4. **Insects 二值化只能称 "ID 分组"**，class ID → 物种/性别无官方映射。
> 5. **真实数据的 5 个 seed 不是独立重复**（此前无 `torch.manual_seed`，已在 `src/utils/seeding.py` 修复）。
>
> **Phase 5.5 的活计划在 [`todo.md`](todo.md)**（含进度日志），本文件只保留背景与架构。
