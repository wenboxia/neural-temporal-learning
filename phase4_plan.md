...
Phase 3（v2 + B + F）在 regime_switching 上 -4.5pp 退步，已 commit 至 6928142。诊断旅程指向 "adapter 跨 regime 学不到 pattern"。
...
## Design E: Dynamic Context Reset（如果 Oracle 通过）
...
## Design A: Regime Detection + Adapter Library（如果 Oracle 不通过）
...
## 论文 framing（Phase 5 用）

根据 Phase 4 走 E 还是 A，论文叙事不同：

**如果走 E（context management 路线）**：
- Title 候选：*Rethinking Online Adaptation for Frozen In-Context Learners: Context Management as the Missing Lever*
- 核心 contribution：证明对 frozen ICL 模型，context lifecycle management > parameter adaptation
- Phase 3 章节：作为"我们尝试了参数化 adapter 路线并系统性证伪"的 narrative，强化 contribution

**如果走 A（adapter library 路线）**：
- Title 候选：*Per-Regime Adapter Libraries for Concept Drift on Frozen Tabular Foundation Models*
- 核心 contribution：用 ADWIN 触发 + adapter 字典实现 "PFC working memory" 隐喻
- Phase 3 章节：作为 "为什么共享 adapter 失败、为什么需要 per-regime 隔离" 的设计动机

两个 framing 都把 Phase 3 的负面发现当作 paper 的 strength（不是 weakness），符合 reviewer 对 "诚实展示设计旅程" 的偏好。
